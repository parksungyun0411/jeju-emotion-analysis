...
class JejuEmotionClassifier:
    ...
    def prepare_data(self, df, text_column='제주어 문장', emotion_column='감정번호'):
        """
        데이터 준비 함수
        """
        print(f"📊 제주어 데이터 준비 중...")
    ...
    def plot_confusion_matrix(self, y_true, y_pred, model_name, dataset_name):
        ...
        plt.title(f'{dataset_name} 제주어 - {model_name} 혼동행렬')
        plt.xlabel('예측 라벨')
        plt.ylabel('실제 라벨')
        
        # 그래프 저장
        os.makedirs('results_jeju', exist_ok=True)
        plt.savefig(f'results_jeju/{dataset_name}_제주어_{model_name}_confusion_matrix.png', 
                    dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"📊 혼동 행렬 저장: results_jeju/{dataset_name}_제주어_{model_name}_confusion_matrix.png")
    
    def save_model(self, model, model_name, dataset_name):
        """
        모델 저장
        """
        os.makedirs('models_jeju', exist_ok=True)
        ...
        filename = f'models_jeju/{dataset_name}_제주어_{model_name}_model.pkl'
    ...
    def save_results(self, results, dataset_name):
        ...
        filename = f'results_jeju/{dataset_name}_제주어_performance_summary.csv'
        performance_df.to_csv(filename, index=False, encoding='utf-8-sig')
        
        print(f"📊 성능 결과 저장됨: {filename}")

def main():
    """
    메인 실행 함수
    """
    print("🎯 제주어 N-gram 기반 감정 분류 모델 학습 및 평가")
    print("=" * 80)
    ...
    for dataset_name, filename in datasets.items():
        ...
        try:
            ...
            print(f"\n✅ {dataset_name} 제주어 처리 완료!")
            print(f"   최고 성능 모델: {best_model_name}")
            print(f"   Accuracy: {best_result['accuracy']:.4f}")
            
        except Exception as e:
            logger.error(f"{dataset_name} 제주어 처리 중 오류 발생: {e}")
    ...
    print(f"\n🏁 모든 데이터셋 제주어 처리 완료!")
    print("📂 생성된 파일들:")
    print("   - models_jeju/ : 학습된 제주어 모델들")
    print("   - results_jeju/ : 성능 비교 CSV 및 혼동행렬 이미지")
    print("   - 베이스라인_성능결과_제주어.txt : 통합 성능 보고서")

def generate_baseline_report(all_results):
    """
    통합 베이스라인 성능 결과 보고서 생성
    """
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    report = f"""🎯 제주어 N-gram 기반 감정 분류 베이스라인 성능 평가 결과
============================================================
...
------------------------------------------------------------"""
        ...
        for model_name, result in results.items():
            report += f"""
{model_name:<20} {result['accuracy']:.4f}      {result['f1_macro']:.4f}      {result['f1_weighted']:.4f}"""
...
🏆 최고 성능 모델: {best_model_name}
🥇 베이스라인 성능: {best_result['accuracy']:.4f} (Accuracy), {best_result['f1_macro']:.4f} (F1-Macro)
"""
...
1. 제주어 텍스트의 감정 분류 성능 확인
2. N-gram 기반 전통적 머신러닝의 한계
3. 표준어 대비 제주어의 분류 난이도
4. 모델별 성능 차이 및 특성
...
📁 저장된 파일들:
- models_jeju/ : 학습된 제주어 모델들
- results_jeju/ : 성능 비교 CSV 및 혼동행렬 이미지
- 베이스라인_성능결과_제주어.txt : 이 보고서

📊 결론:
- 제주어 감정 분류의 베이스라인 성능을 확인했습니다.
- N-gram 기반 전통적 머신러닝의 한계를 보여줍니다.
- 더 정확한 성능을 위해서는 딥러닝 모델이 필요합니다.
- 제주어 특화 전처리 및 모델링 기법이 필요합니다.
"""
    
    # 보고서 저장
    with open('베이스라인_성능결과_제주어.txt', 'w', encoding='utf-8') as f:
        f.write(report)
    
    print(f"📊 통합 베이스라인 성능 결과 저장: 베이스라인_성능결과_제주어.txt")
